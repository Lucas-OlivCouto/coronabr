...
#'
#' @param df Data frame contendo o resultado da busca de `get_corona()`
...
plot_corona_minsaude <- function(df,
  ...
  datas <- plyr::count(df$date[df$cases > 0])
  ...
  df <- df %>%
    ...
    dplyr::summarise_at(dplyr::vars(.data$cases, .data$deaths),
                        .funs = sum, na.rm = TRUE) %>%
    dplyr::filter(., .data$date <= data_max)
  # tipo = numero
  if (tipo == "numero") {
    if (log == TRUE) {
      df <- df %>% dplyr::mutate(cases = log(.data$cases))
      ylab <- paste(ylab, "(log)")
    }
  ...
  if (tipo == "aumento") {
    df$delta_cases <- df$cases - dplyr::lag(df$cases)
    ...
    p <- ggplot2::ggplot(df,ggplot2::aes(x = .data$date, y = .data$delta_cases)) +
      ...
      ggplot2::scale_x_date(date_breaks = "1 day",
                            date_labels = "%d/%m") +
      ggplot2::scale_y_continuous(limits = c(0, max(df$delta_cases, na.rm = TRUE) + 300),
                                  expand = c(0, 0)) +
